"""
Touché 2026 — Training script for ModernBERT-large.
Near drop-in for the RoBERTa template. Key differences:
  - ModernBERT uses unpadding internally (no token_type_ids).
  - Supports long contexts natively (8192 tokens).
  - Needs transformers >= 4.48 for ModernBERT support.
"""
...
def main():
    ...
    print(f"[train] max_len={max_len}, loss={loss_type}, patience={patience}, budget={time_budget}s")
    ...
    print(f"[train] DONE. F1={metrics['f1_macro']:.4f}, "
          f"wall={metrics['wall_clock_s']}s, vram={metrics['peak_vram_mb']}MB")
