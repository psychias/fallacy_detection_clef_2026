"""
Touché 2026 — Training script v3.
Adds per-sample loss weighting for synth data (default 0.5x).
Real examples get weight 1.0, synth examples get synth_weight from config.
"""
...
def main():
    ...
    print(f"[train] synth_weight={synth_weight}")

    # Defensive: ST3 has 17:1 class imbalance (epistemic-internal:practical-external)
    # in the real train data. Plain CE will collapse the model to the majority class
    # and tank macro-F1. Warn loudly if someone forgot to set weighted_ce.
    if subtask == "st3" and loss_type == "ce":
        print("[train] WARNING: subtask=st3 with loss='ce'. Real ST3 has 17:1 class "
              "imbalance — recommend loss='weighted_ce' or 'label_smoothing'.")
