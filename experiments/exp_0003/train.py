"""
Touché 2026 — Training script v2.
Fixes: force fp32 model, disable TF32, NaN-safe training loop,
...
def main():
    ...
    model = AutoModelForSequenceClassification.from_pretrained(
        backbone, num_labels=num_labels, torch_dtype=torch.float32
    )
    model = model.float()  # Force all parameters to fp32
    model.to(device)
    print(f"[train] params={count_params(model):,}, device={device}")
    print(f"[train] model dtype={next(model.parameters()).dtype}")
    ...
    for epoch in range(epochs):
        ...
        for batch in train_loader:
            ...
            if not math.isfinite(loss.item()):
                nan_count += 1
                print(f"[train] WARNING: NaN/Inf loss at step {global_step}, skipping batch (total NaN: {nan_count})")
                optimizer.zero_grad()
                if nan_count > 50:
                    status_flags.append("excessive_nan")
                    print("[train] ERROR: >50 NaN batches, aborting training")
                    time_exceeded = True  # triggers exit
                    break
                global_step += 1
                continue
        ...
        # Count unique predictions for degenerate-model detection
    ...
    if best_ckpt.exists():
        model = AutoModelForSequenceClassification.from_pretrained(
            str(best_ckpt), num_labels=num_labels, torch_dtype=torch.float32
        )
        model = model.float()
        model.to(device)
        model.eval()
    ...
    # Clamp inf/nan for JSON serialization
    ...
    print(f"[train] DONE. F1={metrics['f1_macro']:.4f}, "
          f"wall={metrics['wall_clock_s']}s, nan_batches={nan_count}")
