...
The largest single gain came on ST3 base, where the \texttt{synth\_v002} recipe at \texttt{max\_len=384} lifted F1 from 0.44 to 0.52, an improvement of +0.087. % from exp_0058 exp_0069
...
On ST3 enhanced the same recipe at \texttt{max\_len=384} lifted F1 to 0.74, a gain of +0.041 over the closest matching synth-only baseline. % from exp_0073
...
That number was what motivated the \textsf{pe}-targeted synth batch above. After training the final ST3-enhanced classifier, we ran a per-class threshold sweep on the dev partition; the best configuration $\{\text{ee}=0.1, \text{ei}=0.1, \text{pe}=0.05, \text{pi}=0.4\}$, added +1.0\,pp F1 over default argmax. % from exp_0145
