...
%% ---------------------------------------------------------------------
\title{UZHCL\_for\_the\_win at Touch\'{e} 26: Encoder Fine-Tuning,
  Synthetic Augmentation, and an LLM Detour}
\title[mode=sub]{Notebook for the Touch\'{e} Lab at CLEF 2026}
...
%% ---------------------------------------------------------------------
\begin{abstract}
Online discussions on platforms like Reddit are a productive testbed for fallacy detection: short texts, broad topical range, and the full inventory of informal fallacies catalogued by classical argumentation theory long before social media. The Touch\'{e} 2026 lab at CLEF formalises this as a shared task on a Reddit-derived corpus and provides a structured benchmark for systems that combine fallacy detection with argument-scheme classification. This paper describes our participation in the Touch\'{e} 2026 Fallacy Detection lab, covering all three subtasks (ST1 binary fallacy detection, ST2 eight-way fallacy classification, ST3 four-way argument-scheme classification) and both evaluation tracks. Our submitted systems are fine-tuned RoBERTa-large encoders with task-specific heads, augmented with a small batch of LLM-generated synthetic contrastive pairs. For ST2 and ST3 on the enhanced track we additionally extended training with a pseudo-labelled top-up of the published unlabelled pool, gated by a leak-audit step; we ran a wider backbone grid covering DeBERTa-v3, ModernBERT, and four Qwen variants (3B zero-shot/few-shot, 7B and 32B CoT-LoRA, and Qwen3-4B-Thinking) for comparison. A key consideration for this year's submission was the structural status of the enhanced-track fields, which were rewritten by the lab organisers with knowledge of the gold labels and which therefore shaped both our system design and our interpretation of results. On our held-out development partitions our systems perform competitively across all six (subtask, track) cells, with the strongest results on the enhanced track of ST2. We discuss these outcomes in light of the enhanced-track leakage question and the small-corpus limits of the encoder-vs-LLM comparison, and outline directions for future work. All code is publicly available at \url{https://github.com/psychias/fallacy_detection}.
\end{abstract}
...
\input{sections/02_previous_work.tex}
...
\paragraph{Prompt 1: ST1/ST2 contrastive-pair generation
(\texttt{synth\_v002} ST1/ST2; \texttt{synth\_v003} rare-class top-up).}
System message:
\begin{verbatim}
...
("idk", "tbh", "iirc"), occasional typos, no debate-club phrasing,
no signposting. Never name the fallacy inside the generated text.
...
\paragraph{Prompt 2: ST3 contrastive-pair generation
(\texttt{synth\_v002} ST3).}
Same shape as Prompt 1, but produces two NON-fallacious replies using
different argumentation schemes. System message:
\begin{verbatim}
...
\paragraph{Prompt 3: \textsf{pe}-targeted generation (\texttt{exp\_0103}).}
A separate, simpler prompt generated the pe-targeted batch (50
examples per label, \texttt{openai/gpt-4o-mini}, temperature 0.9).
System message:
\begin{verbatim}
...
\begin{verbatim}
[pe]  Generate a short argumentative text (2-4 sentences) that uses
      propaganda or emotional manipulation. Include at least one of:
      loaded language, appeal to fear, bandwagon appeal, false
      dilemma, or emotional blackmail. Make it sound like real
      political/media discourse.

[pa]  Generate a short argumentative text (2-4 sentences) that
      primarily appeals to emotion (pathos) rather than logic. Use
...
predictions at $\tau=0.9$ (see \S\ref{sec:pseudo}); it is not LLM-API
...
\midrule
ST1 & base     & exp\_0033 & 256 & 16 & 1e-5 & 15 & synth\_v002 sw=0.5 \\ % from exp_0033
ST1 & enhanced & exp\_0035 & 256 & 16 & 1e-5 & 12 & synth\_v002 sw=0.5 \\ % from exp_0035
ST2 & base     & exp\_0032 & 256 & 16 & 1e-5 & 15 & synth\_v002 sw=0.5 \\ % from exp_0032
ST2 & enhanced & exp\_0072 & 384 & 16 & 1e-5 & 12 & synth + pseudo sw=0.7 \\ % from exp_0072
ST3 & base     & exp\_0069 & 384 & 16 & 1e-5 & 15 & synth\_v002 \\ % from exp_0069
ST3 & enhanced & exp\_0073 & 384 & 16 & 1e-5 & 12 & synth + pseudo sw=0.7 \\ % from exp_0073
\bottomrule
\end{tabular}
\end{table}
...
(\texttt{exp\_0145}, $+1.0$\,pp F1 over default argmax on the dev partition). % from exp_0145
...
development partition. ST2 values are computed from default argmax
predictions of \texttt{exp\_0072}; ST3 values are computed from
\texttt{exp\_0073} under the per-class thresholds from \texttt{exp\_0145}
($\text{ee}{=}0.1, \text{ei}{=}0.1, \text{pe}{=}0.05, \text{pi}{=}0.4$).
Both macro-F1 values reconcile with Table~\ref{tab:headline}
(\S\ref{sec:results}) within $\pm 0.001$ rounding.

\begin{table}[h]
\centering
\small
\caption{Per-class precision, recall, F1, and support on the dev
partition for ST2-enhanced (\texttt{exp\_0072}) and ST3-enhanced
(\texttt{exp\_0073}). Computed from saved prediction artefacts. ST3
cells reflect the per-class threshold calibration of \texttt{exp\_0145}.}
\label{tab:perclass}
\begin{tabular}{lcccc}
\toprule
Label & Precision & Recall & F1 & Support \\
\midrule
\multicolumn{5}{l}{\textit{ST2 (8-way; exp\_0072; argmax)}} \\ % from exp_0072
authority             & 0.972 & 0.986 & 0.979 & 72 \\
black\_white          & 0.967 & 0.967 & 0.967 & 30 \\
hasty\_generalization & 0.964 & 0.958 & 0.961 & 72 \\
natural               & 1.000 & 1.000 & 1.000 & 21 \\
population            & 0.948 & 0.971 & 0.959 & 68 \\
slippery\_slope       & 1.000 & 0.957 & 0.978 & 46 \\
tradition             & 0.971 & 0.971 & 0.971 & 35 \\
worse\_problems       & 0.940 & 0.957 & 0.948 & 47 \\
\midrule
\multicolumn{5}{l}{\textit{ST3 (4-way; exp\_0073; thresholds from exp\_0145)}} \\ % from exp_0073 + exp_0145
...
underperformance, and the LLM detour that did not validate under CV).
...
\node[blue,  below=of c2] (c3)               {Heinrich et al.\ 2026\\\textit{Lab overview}};
...
\node[pam, right=of c9, yshift=-3mm]  (p3) {Synth + pseudo augmentation\\\textit{sw 0.5--0.7}};
...
underperforming RoBERTa-large, and the LLM detour that did not
