"""
Touché 2026 — Joint ST1+ST2 Training script.
9-way classification: non-fallacy + 8 fallacy types.
At inference, ST1 prediction = (argmax != non-fallacy), ST2 prediction = argmax label.
Based on train_v4 with all features (seed, precision, focal, grad_clip).
"""
...
def main():
    ...
    print(f"[joint] exp_dir={exp_dir.name}, subtask={subtask}, track={track}")
    print(f"[joint] backbone={backbone}, lr={lr}, bs={batch_size}, epochs={epochs}")
    print(f"[joint] num_labels={num_labels} (non-fallacy + 8 fallacy types)")
    print(f"[joint] precision={precision}, grad_clip={grad_clip}, seed={seed}")
    ...
    # Synthetic data (ST1+ST2 synth repurposed as joint labels)
    ...
    for sv in synth_versions:
        try:
            ...
            print(f"[joint] WARNING: synth version {sv} not found, skipping")
    ...
    print(f"[joint] train={len(train_entries)} (real={n_real}, synth={n_synth}), "
          f"dev={len(dev_entries)}")
    train_dist = Counter(e["label"] for e in train_entries)
    print(f"[joint] train_dist={dict(sorted(train_dist.items()))}")
    ...
    model.to(device)
    print(f"[joint] params={count_params(model):,}, device={device}")
    ...
    if use_amp:
        print(f"[joint] AMP enabled: precision={precision}, dtype={amp_dtype}")

    # ── Early stopping ────────────────────────────────────────────
    patience = train_cfg.get("patience", 0)
    if patience > 0:
        print(f"[joint] early_stopping: patience={patience}")
    ...
    for epoch in range(epochs):
        ...
        # ── Dev evaluation (joint + decomposed ST1/ST2 metrics) ───
        model.eval()
        ...
        joint_metrics = evaluate(y_true, y_pred, subtask)

        # Decompose into ST1 (binary) and ST2 (8-way on fallacious subset)
        st1_true = ["fallacy" if y != "non-fallacy" else "non-fallacy"
                     for y in y_true]
        st1_pred = ["fallacy" if y != "non-fallacy" else "non-fallacy"
                     for y in y_pred]
        st1_metrics = evaluate(st1_true, st1_pred, "st1")

        # ST2: only entries where ground truth is fallacious
        st2_mask = [y != "non-fallacy" for y in y_true]
        st2_true = [y for y, m in zip(y_true, st2_mask) if m]
        st2_pred = [y for y, m in zip(y_pred, st2_mask) if m]
        st2_metrics = evaluate(st2_true, st2_pred, "st2") if st2_true else {
            "f1_macro": 0.0}

        dev_f1 = joint_metrics["f1_macro"]
        print(f"[joint] epoch={epoch}, loss={avg_loss:.4f}, "
              f"joint_f1={dev_f1:.4f}, "
              f"st1_f1={st1_metrics['f1_macro']:.4f}, "
              f"st2_f1={st2_metrics['f1_macro']:.4f}")

        if dev_f1 > best_dev_f1:
            best_dev_f1 = dev_f1
            no_improve = 0
            save_checkpoint(model, tokenizer, exp_dir)
            print(f"[joint] New best joint F1: {best_dev_f1:.4f}, checkpoint saved")
        else:
            no_improve += 1
    ...
    if best_ckpt.exists():
        ...
        # Decomposed ST1 / ST2 final metrics
        st1_true = ["fallacy" if y != "non-fallacy" else "non-fallacy"
                     for y in y_true]
        st1_pred = ["fallacy" if y != "non-fallacy" else "non-fallacy"
                     for y in y_pred]
        metrics["st1_f1"] = evaluate(st1_true, st1_pred, "st1")["f1_macro"]

        st2_mask = [y != "non-fallacy" for y in y_true]
        st2_true = [y for y, m in zip(y_true, st2_mask) if m]
        st2_pred = [y for y, m in zip(y_pred, st2_mask) if m]
        if st2_true:
            metrics["st2_f1"] = evaluate(st2_true, st2_pred, "st2")["f1_macro"]
    ...
    print(f"[joint] DONE. Joint F1={metrics['f1_macro']:.4f}, "
          f"ST1 F1={metrics.get('st1_f1', 'N/A')}, "
          f"ST2 F1={metrics.get('st2_f1', 'N/A')}, "
